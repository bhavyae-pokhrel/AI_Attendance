...
def login_teacher(username,password):
    ...
    if teacher:
        ...
        st.session_state_is_logged_in = True
        return True
    else:
        st.toast("Invalid username or password!", icon="⚠️")
        return False
...
def teacher_screen_login():
    ...
    with btnc1:
        if st.button('Login', icon=':material/passkey:', shortcut='control+enter', width='stretch', key='login_btn'):
...
def register_teacher(teacher_username,teacher_name,teacher_pass,teacher_pass_confirm,):
    ...
    if check_teacher_exists(teacher_username):
        return False, "Teacher username already exist"
    
    if teacher_pass != teacher_pass_confirm:
        return False, "Password and Confirm Password are not match"
